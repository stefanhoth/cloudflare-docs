...
Workers often need access to data not in the `request` object. For example, sometimes Workers use [`waitUntil`](/runtime-apis/fetch-event#waituntil) to delay execution. Module workers can access `waitUntil` via the `context` parameter. For a list of Module worker parameters, see [this list](/runtime-apis/fetch-event#parameters).
